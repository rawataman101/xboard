...
async function getFeed(i) {
  try {
    const res = await fetch(
      `https://api.rss2json.com/v1/api.json?rss_url=${magazines[i]}`
    );
...
function getCarousel(data, index) {
  ...
  document
    .getElementById(`carousel__card${index + 1}`)
...
function getCard(i) {
  ...
  cardDiv.innerHTML = `
      <img class="card-img-top" src="${i.enclosure.link}" alt="Card image cap">
      <div class="card-body">
